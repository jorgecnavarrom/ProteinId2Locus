...
def parameter_parser():
    ...
    parser.add_argument("-l", "--list", help="File with protein accession list", 
        type=Path)
    parser.add_argument("-p", "--proteinid", help="A single protein id to query",
        type=str)
...
def download_genbank(output, original_acc, best_candidate, extra):
    if best_candidate.nuc_acc == "":
        ...
        filename = f"{original_acc}_{current_best.new_acc}_protein.fasta"
        handle = Entrez.efetch(db="protein", id=best_candidate.new_acc, rettype="fasta", retmode="text")
        ...
        filename = f"{original_acc}_{current_best.new_acc}_locus.gbk"
        ...
        handle = Entrez.efetch(db="nucleotide", id=best_candidate.nuc_acc, seq_start=str(start), seq_stop=str(end), rettype="gb", retmode="text")
        with open(output / filename, "w") as gb:
            gb.write(handle.read())
        return f"{filename}\t{current_best.new_acc}"
...
if __name__ == '__main__':
    ...
    # Validation: doesn't work too well as each 'post' generates a different query_key
    # and then Entrez.efetch will only get results for a single query
    ...
    # Query everything else in the same 'post' + try to detect anomalies with accessions
    ...
    with open(o / "ipg_results.txt", "w") as ipg_res, open(o / "accession_map.tsv", "w") as am:
        for acc, qk in acc_to_qk.items():
            iden_prots = Entrez.efetch(db="protein", rettype='ipg', retmode='xml', WebEnv=we, query_key=qk)
            sleep(0.34)
            try:
            ...
            for report in ipgs.values():
                ...
                for prot in report["ProteinList"]:
                    ...
                    if "CDSList" in prot:
                        for cds in prot["CDSList"]:
                            ...
                            ipg_res.write(f"{acc}\t{source}\t{nuc_acc}\t{seq_start}\t{seq_stop}\n")

                            candidate = acc_data(na=acc, src=source, nuca=nuc_acc, begin=seq_start, end=seq_stop)
                ...
                filename_currentacc = download_genbank(o, original_acc, current_best, args.extra)
